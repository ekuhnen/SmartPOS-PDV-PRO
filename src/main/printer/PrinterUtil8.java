...
public class PrinterUtil8 {
    ...
    private static final int SECTION_SPACING_ = 12;
    private static final int BOTTOM_FEED = 140;

    // Colunas ajustadas
    private static final int LABEL_COL_WIDTH = 145;
                ...
                try {
                    ...
                    try {
                        ...
                        if (bitmap != null) {
                            ...
                            if (alturaImpressa > 0) {
                                y += alturaImpressa + SECTION_SPACING_;
                            } else {
                                y += bitmap.getHeight() + SECTION_SPACING_; // Fallback de segurança se retornar -1
                            }
                        }
                    } catch (Exception e) {
                    ...
                    y = printTextCentered(pm, titulo, y, DEFAULT_FONT, tamanhoFonte1, SECTION_SPACING_);
                    y += 5; // ou 5, ou 0
    ...
    private static int drawLabelValueInternal(PrinterManager pm, String label, String value, int y, String font, int size, boolean bold) {
        ...
        // O driver não suporta alinhamento à direita nativo.
        // A melhor abordagem é alinhar à esquerda na coluna de valor e usar word wrap
        // ou calcular espaços manualmente. Aqui usamos a coluna de valor com word wrap.
        ...
        for (String line : valueLines) {
            // Se quiséssemos alinhar estritamente à direita, teríamos que calcular a largura em pixels
            // da string e subtrair de VALUE_COL_WIDTH. Como não temos acesso à largura exata,
            // desenhamos na posição VALUE_COL_X.
            int h = pm.drawTextEx(
                    line,
                    VALUE_COL_X,
                    currentY,
